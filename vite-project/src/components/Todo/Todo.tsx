...
const Todo = () => {
    const [tast, setNameTask] = useState<string>('')
    const [listToDo, setListToDo] = useState<[] | IListToDo[]>([])
    const [done, setDone] = useState<string>("All")
    ...
    const addToDoHendler = (e: React.KeyboardEvent<Element>):void => {
        if(e.key === 'Enter'){
            e.preventDefault()
    ...
    const isChackedInput = (index:number):void => {
        const newList  = ([] as any[]).concat(listToDo) 
